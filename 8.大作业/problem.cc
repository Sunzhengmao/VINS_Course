...
bool Problem::AddVertex(std::shared_ptr<Vertex> vertex) {
    ...
    if (problemType_ == ProblemType::SLAM_PROBLEM) {
        if (IsPoseVertex(vertex)) {
            ResizePoseHessiansWhenAddingPose(vertex);/// 如果是位姿的话, 就对H_prior_, b_prior_进行扩维，并将没有值的地方设为0(imu与cam之间也算pose)
        }
    }
    return true;
}
...
/// H_prior_, b_prior_进行扩维，并将没有值的地方设为0
...
vector<shared_ptr<Edge>> Problem::GetConnectedEdges(std::shared_ptr<Vertex> vertex) {
    ...
    auto range = vertexToEdge_.equal_range(vertex->Id());/// 114->在sorted vertexToEdge_里面找出与Id一样的值, 返回两个iterator组成的pair
...
bool Problem::Solve_new(int iterations, OptimizationFlag optimization_method)
{
...
//    std::fstream time_data;
//    string path = "/home/sunzhengmao/VIO/lesson8/BigHW/LM_newLM/time/time_new_LM.txt";
//    time_data.open(path, ios::app);
//    if (!time_data.is_open())
//    {
//        cerr<<"there are some wrong happened at open time_data!!"<<endl;
//    }

    std::fstream time_makeHessian;
    string mH_path = "/home/sunzhengmao/VIO/lesson8/BigHW/OpenMP/0thread_4.txt";
    time_makeHessian.open(mH_path, ios::app);
    if(!time_makeHessian.is_open())
    {
        cerr<<"Something went wrong!"<<endl;
    }

//    std::fstream last_chi;
//    string chi_path = "/home/sunzhengmao/VIO/lesson8/BigHW/LM_newLM/last_chi/chi_new_LM.txt";
//    last_chi.open(chi_path, ios::app);
//    if(!last_chi.is_open())
//    {
//        cerr<<"Something went wrong!!"<<endl;
//    }
//
//    std::fstream iter_num;
//    string iter_path = "/home/sunzhengmao/VIO/lesson8/BigHW/LM_newLM/iter_num/iter_new_LM.txt";
//    iter_num.open(iter_path, ios::app);
//    if(!iter_num.is_open())
//        cerr<<"Something went wrong!!"<<endl;
...
//        time_data << t_solve.toc()<<endl;
//        time_makeHessian << t_hessian_cost_ << endl;
//        last_chi << currentChi_<<endl;
//        iter_num << iter <<endl;

        all_time_in_H += t_hessian_cost_;
        std::cout << "   all time used for makeHessian cost: " << all_time_in_H << " ms" << std::endl;
        t_hessian_cost_ = 0.;
        return true;
    }


    if (optimization_method == DogLeg)
    {
        //DogLeg initialization
        /// 计算初始值
        ComputeParamInitDL();
        ...
        while (!stop && (iter<iterations))
        {
            std::cout << "iter: " << iter << " , chi= " << currentChi_ << " , radius= " << trust_region << std::endl;
            std::cout<<"delta_x_.norm() is: "<<delta_x_.norm()<<endl;

            /// get alpha and H_sd
            SolveLinearSystemInSD();

            bool oneStepSuccess = false;
            bool GNisSolved = false;
            int false_cnt = 0;

            while (!oneStepSuccess && false_cnt < 10)  // 不断尝试 DogLeg, 直到成功迭代一步
            {
                if(H_sd.norm() >= trust_region)
                    delta_x_ = trust_region / H_sd.norm() * H_sd;
                else
                {
                    if (! GNisSolved)
                    {
                        SolveLinearSystemInGN();
                        GNisSolved = true;
                    }
                    if(H_gn.norm() <= trust_region)
                        delta_x_ = H_gn;
                    else
                    {
                        /// 取交点
                        ...
                        if (c <= 0)
                            beta = (-c + sqrt(c * c + (b - a).squaredNorm() * (trust_region * trust_region - a.transpose() * a))) /
                                    (b - a).squaredNorm();
                        else
                            beta = (trust_region * trust_region - a.squaredNorm()) /
                                    (c + sqrt(c * c + (b - a).squaredNorm() * (trust_region * trust_region - a.squaredNorm())));

                        delta_x_ = a + beta * (b - a);
                    }
                }

                /// 用每个小的优化变量的模之和近似大的优化变量的模, 用三角不等式得只会更小不会变大, 满足要求
                norm_Parameters = 0.;
                for (auto vertex = verticies_.begin(); next(next(vertex)) != verticies_.end(); vertex++)
                {
                    norm_Parameters += vertex->second->Parameters().transpose().norm();
                }

                /// 优化退出条件1：H_dl太小了
                if(delta_x_.norm() <= xigma2 * (norm_Parameters + xigma2))
                {
                    cout<<"residual is too small, so exit!"<<endl;
                    stop = true;
                    break;
                }
                ...
                /// 优化退出条件3： 信赖域太小了
                if (trust_region < xigma2 * (norm_Parameters + xigma2))
                {
                    stop = true;
                    cout<<"trust_region is too small, so exit!"<<endl;
                    break;
                }
            }
            iter++;
            ...
            /// 优化退出条件4：currentChi_ 跟第一次的 chi2 相比，下降了 1e6 倍则退出
            if(last_chi_ - currentChi_ < 1e-5)
            {
                std::cout << "sqrt(currentChi_) <= stopThresholdLM_" << std::endl;
                stop = true;
        ...
        if(iter == iterations)
            cout<<"iter is equal to iterations, so exit!"<<endl;
        std::cout << "problem solve cost: " << t_solve.toc() << " ms" << std::endl;
        std::cout << "   makeHessian cost: " << t_hessian_cost_ << " ms" << std::endl;
//        time_data << t_solve.toc()<<endl;
        time_makeHessian << t_hessian_cost_ << endl;
...
//#pragma omp parallel for num_threads(2)
    for (auto &edge: edges_)
    {
        ...
        /// 把每个残差都加起来
        norm_chi += edge.second->Residual().norm();
...
//    double rou_tilde_ = 0.0;
//    rou_tilde_ = abs(thita1 * pow(norm_chi, a_new) + thita2 * pow((b.norm()), b_new));
//    rou_tilde = rou_tilde_ <= 1 ? rou_tilde_ : 1;

    if(H_prior_.rows() > 0)///先验信息也要加入到hessian矩阵的求解中来
    {
...
/*
 * Solve H_dl, we will choose h_gn or h_sd as the result of h_dl
 */
void Problem::SolveLinearSystemInSD()
{
 ...
 */
void Problem::SolveLinearSystemInGN()
{
...
void Problem::SolveLinearSystem()
{
    if (problemType_ == ProblemType::GENERIC_PROBLEM)
    {
        ...
        MatXX H = Hessian_;
        for (size_t i = 0; i < Hessian_.cols(); ++i) {
            H(i, i) += currentLambda_*rou_tilde;
        }
        // delta_x_ = PCGSolver(H, b_, H.rows() * 2);
    ...
    else
    {
        ...
        // step2: solve Hpp * deltax = bpp
        VecX delta_x_pp(VecX::Zero(reserve_size));

        for (ulong i = 0; i < ordering_poses_; ++i)
        {
            H_pp_schur_(i, i) += currentLambda_ ;//* rou_tilde;              // LM Method
        }
...
/// DogLeg
void Problem::ComputeParamInitDL()
{
    currentChi_ = 0.0;
    trust_region = 1.0;
...
bool Problem::IsGoodStepInDL()
{
    ...
    else if(delta_x_ == trust_region / H_sd.norm() * H_sd)
    {
        scale = trust_region * (2 * alpha * (-b_).norm() - trust_region) / (2 * alpha);

    }
    else
    {
        scale = alpha * (1 - beta) * (1 - beta) * (-b_).squaredNorm() /2 + beta * (2 - beta) * currentChi_;
    ...
    //update trust_region
    if (rho_ > 0.75)
        trust_region = max(trust_region, 3*delta_x_.norm());
    else
    {
        if (rho_ < 0.25)
        {
...
//            }
    ...
    if (rho_ > 0 && isfinite(tempChi))   // last step was good, 误差在下降
    {
        currentChi_ = tempChi;
        return true;
    }
...
//    if (rho_ > p2 && rho_ > 0 && isfinite(tempChi))
//    {
//        currentLambda_ *= q1;
//    }
//    else
//    {
//        if (rho_ < p1)
//            currentLambda_ = max(q2*currentLambda_, m);
//    }
//
//    if (rho_ > p0 && isfinite(tempChi))   // last step was good, 误差在下降
//    {
//        currentChi_ = tempChi;
//        return true;// 近似的好
//    }
...
//    }
    ...
    if(rho_ > 1e-9 && isfinite(tempChi))
    {
        currentLambda_ = max(currentLambda_/L_down, 1e-7);
        currentChi_ = tempChi;
        return true;
    }
    else
    {
        currentLambda_ = min(currentLambda_*L_up, 1e7);
        return false;
    }
}
...
bool Problem::Marginalize(const std::vector<std::shared_ptr<Vertex> > margVertexs, int pose_dim) {
    ...
    std::vector<shared_ptr<Edge>> marg_edges = GetConnectedEdges(margVertexs[0]);///找到margVertex[0] = pose 相连的landmark
...
//      std::cout << "marg edge id: "<< marg_edges[i]->Id() <<std::endl;
        ...
        for (auto iter : verticies) {///对每个边连接的每个顶点, 把landmark的id放在pose后面, 并添加margLandmark和维度
...
//    cout<<"1 Hessian's dimension is: "<<cols<<"x"<<cols<<endl;
    ...
    for (auto edge: marg_edges) {
        ...
        auto jacobians = edge->Jacobians();/// 对这个edge的四个顶点都进行求导，放到vector(jacobians)中
        auto verticies = edge->Verticies();/// 这个边连接的四个顶点, 应该是设计好的, 与jacobians对应好了
        ii++;

        /// 也可以直接把jacobian拼成nx1, 再算hessian(nxn), 但是也可以对hessian的16个小块分别计算,
        /// 最后在把他们拼接起来, 而且由于是对称的, 所以只需要算一半就行了(8个)
        assert(jacobians.size() == verticies.size());
        for (size_t i = 0; i < verticies.size(); ++i) {
            auto v_i = verticies[i];
            auto jacobian_i = jacobians[i];
...
//    cout<<"2 Hessian's dimension is: "<<H_marg.rows()<<"x"<<H_marg.cols()<<endl;
...
//        std::cout << k << " "<<idx << std::endl;
    ...
    Eigen::MatrixXd Amm_inv = saes.eigenvectors() * Eigen::VectorXd(///特征向量
            (saes.eigenvalues().array() > eps).select(saes.eigenvalues().array().inverse(), 0)).asDiagonal() *///特征值的逆
                              saes.eigenvectors().transpose();///特征向量的逆, 得到Amm的逆
...
//    cout<<"3 Hessian's dimension is: "<<H_prior_.rows()<<"x"<<H_prior_.cols()<<endl;

    /// 用一种看不懂的方式重新算了一遍H_prior和b_prior
    Eigen::SelfAdjointEigenSolver<Eigen::MatrixXd> saes2(H_prior_);
    Eigen::VectorXd S = Eigen::VectorXd((saes2.eigenvalues().array() > eps).select(saes2.eigenvalues().array(), 0));
//    cout<<"S is: "<<S.transpose()<<endl;
    Eigen::VectorXd S_inv = Eigen::VectorXd(
            (saes2.eigenvalues().array() > eps).select(saes2.eigenvalues().array().inverse(), 0));
//    cout<<"S_inv is: "<<S_inv.transpose()<<endl;

    Eigen::VectorXd S_sqrt = S.cwiseSqrt();///对S中的每个值取逆
//    cout<<"S_sqrt is: "<<S_sqrt.transpose()<<endl;
    Eigen::VectorXd S_inv_sqrt = S_inv.cwiseSqrt();
//    cout<<"S_inv_sqrt is: "<<S_inv_sqrt.transpose()<<endl;
    Jt_prior_inv_ = S_inv_sqrt.asDiagonal() * saes2.eigenvectors().transpose();///根号S * V.transpose() = J
    err_prior_ = -Jt_prior_inv_ * b_prior_;

    MatXX J = S_sqrt.asDiagonal() * saes2.eigenvectors().transpose();///根号S * V.transpose() = J
    H_prior_ = J.transpose() * J;
    MatXX tmp_h = MatXX( (H_prior_.array().abs() > 1e-9).select(H_prior_.array(),0) );
    H_prior_ = tmp_h;

//    cout<<"4 Hessian's dimension is: "<<H_prior_.rows()<<"x"<<H_prior_.cols()<<endl;
