...
bool Problem::AddVertex(std::shared_ptr<Vertex> vertex) {///这里用父类的指针,寓意调用父类的方法,子类也可以用
...
{
    if (edges_.find(edge->Id()) == edges_.end()) /// 如果没找到就直接到了end
    {
...
Problem::GetConnectedEdges(std::shared_ptr<Vertex> vertex)
{
...
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        ...
        int pose_size = ordering_poses_;
        int landmark_size = ordering_landmarks_;

        // TODO:: home work. 完成矩阵块取值，Hmm，Hpm，Hmp，bpp，bmm

        MatXX Hpp = Hessian_.block(0, 0, pose_size, pose_size);
            for (int i = 0; i < pose_size; ++i)
            {
                Hpp(i, i) += currentLambda_;

            }
        MatXX Hpl = Hessian_.block(0, pose_size, pose_size, landmark_size);
        MatXX Hlp = Hessian_.block(pose_size, 0, landmark_size, pose_size);
        MatXX Hll = Hessian_.block(pose_size, pose_size, landmark_size, landmark_size);

//            for (int j = 0; j < landmark_size; ++j) {
//                Hll(j, j) += currentLambda_;
//            }
        VecX bp = b_.segment(0, pose_size);
        VecX bl = b_.segment(pose_size, landmark_size);////

        MatXX Hll_inv(MatXX::Zero(landmark_size, landmark_size));
        for (auto landmarkVertex : idx_landmark_vertices_)
        {
            int idx = landmarkVertex.second->OrderingId() - pose_size;
            int size = landmarkVertex.second->LocalDimension();
            Hll_inv.block(idx, idx, size, size) = Hll.block(idx, idx, size, size).inverse();
        }////

//        MatXX tempH = Hpm * Hmm_inv;
        H_pp_schur_ = Hpp - Hpl * Hll_inv * Hlp;
        b_pp_schur_ = bp - Hpl * Hll_inv * bl;

        VecX delta_x_pp(pose_size);
//        for (ulong j = 0; j < pose_size; ++j)
//        {
//            H_pp_schur_(j, j) += currentLambda_;
//        }

        int n = H_pp_schur_.rows() * 2;                       // 迭代次数
        delta_x_pp = PCGSolver(H_pp_schur_, b_pp_schur_, n);  // 哈哈，小规模问题，搞 pcg 花里胡哨
        delta_x_.head(pose_size) = delta_x_pp;

        VecX delta_x_ll(landmark_size);
        delta_x_ll = Hll.inverse() * (bl - Hlp * delta_x_pp);
        delta_x_.tail(landmark_size) = delta_x_ll;


//      int reserve_size = ordering_poses_;
//      int marg_size = ordering_landmarks_;

//        MatXX Hmm = Hessian_.block(0, 0, marg_size, marg_size);
//        MatXX Hpm = Hessian_.block(marg_size, 0, reserve_size, marg_size);
//        MatXX Hmp = Hessian_.block(0, marg_size, marg_size, reserve_size);
//        VecX bpp = b_.segment(marg_size, 0);
//        VecX bmm = b_.segment(0, marg_size);

        // Hmm 是对角线矩阵，它的求逆可以直接为对角线块分别求逆，如果是逆深度，对角线块为1维的，则直接为对角线的倒数，这里可以加速
//        MatXX Hmm_inv(MatXX::Zero(marg_size, marg_size));
//        for (auto landmarkVertex : idx_landmark_vertices_)
//        {
//            int idx = landmarkVertex.second->OrderingId() - reserve_size;// pose(reserve)在左上角
//            int size = landmarkVertex.second->LocalDimension();
...
//        MatXX tempH = Hpm * Hmm_inv;
//        H_pp_schur_ = Hessian_.block(marg_size, marg_size, reserve_size, reserve_size) - Hpm * Hmm_inv * Hmp;
//        b_pp_schur_ = bpp - Hpm * Hmm_inv * bmm;
//
//        ///step2: solve Hpp * delta_x = bpp
//        VecX delta_x_pp(VecX::Zero(reserve_size));// dynamic vector
//        // PCG Solver
...
void Problem::TestMarginalize() {
    ...
    Eigen::MatrixXd Arm = H_marg.block(0, n2, n2, dim);
    Eigen::MatrixXd Amr = H_marg.block(n2, 0, dim, n2);
    Eigen::MatrixXd Arr = H_marg.block(0, 0, n2, n2);

//    Eigen::MatrixXd tempB = Arm * Amm_inv;
    Eigen::MatrixXd H_prior = Arr - Arm * Amm_inv * Amr;
