...
int main() {
    ...
    {
        shared_ptr<VertexPose> vertexCam(new VertexPose());/// 构造在这, 什么参数也不需要, 就是(7,6), 完事每次调用一次就id自动增加的
...
//        double inverse_depth = 1. / Pc.z();
        ...
        {
            /// 第j个相机与第0个相机之间的像素坐标来构成投影误差
            /// 来优化相机与相机之间的误差吧，不不不应该也有对路标点的优化
            Eigen::Vector3d pt_i = cameras[0].featurePerId.find(i)->second;
            Eigen::Vector3d pt_j = cameras[j].featurePerId.find(i)->second;


            /// 边不是简单的一条边,还要设置其链接的顶点,残差,雅克比
            shared_ptr<EdgeReprojection> edge(new EdgeReprojection(pt_i, pt_j));
            edge->SetTranslationImuFromCamera(qic, tic);

            std::vector<std::shared_ptr<Vertex> > edge_vertex;

            /// 这里是为了防止一会再便利一遍路标点，直接在一个坐标点下，让每一个camera都可以观测到
            edge_vertex.push_back(verterxPoint);     /// inverse depth
            edge_vertex.push_back(vertexCams_vec[0]);/// camera 0
            edge_vertex.push_back(vertexCams_vec[j]);/// camera j
    ...
    VecX delt = cameras[0].twc - vertexCams_vec[0]->Parameters().head(3);
    std::cout<<"------------ pose translation in vins ----------------"<<std::endl;
    for (int i = 0; i < vertexCams_vec.size(); ++i) {
        std::cout<<"translation after opt: "<< i <<" :"<< vertexCams_vec[i]->Parameters().head(3).transpose() + delt.transpose() << " || gt: "<<cameras[i].twc.transpose()<<std::endl;
    }
